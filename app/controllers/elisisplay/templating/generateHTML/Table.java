package controllers.elisisplay.templating.generateHTML;
...
public class Table {
    ...
    public static  <M>  String tableHTML(List<M> data, String tableClass, String tableId,
                                         boolean onlyPublic, Map<String, String> tablesNaming, String... unless) {

        String table = "<table " + "id=\"" + tableId + "\" class=\"" + tableClass + "\">" +
                "<thead><tr>";
        StringBuilder tableBuilder = new StringBuilder(table);

        HashMap<String, Object> dataMap = Utils.dataTable(new ArrayList<>(data), onlyPublic, unless);
        ArrayList<String> attributs = (ArrayList<String>) dataMap.get("attributs");
        ArrayList<ArrayList<Object>> values = (ArrayList<ArrayList<Object>>) dataMap.get("values");

        for (String string:
             attributs) {
            tableBuilder.append("<th>").append(replaceName(tablesNaming, string)).append("</th>");
        }
        tableBuilder.append("</tr></thead><tbody>");

        for (ArrayList<Object> tr : values){
            tableBuilder.append("<tr>");
            for (Object object:
                 tr) {
                tableBuilder.append("<td>");
                tableBuilder.append(object);
                tableBuilder.append("</td>");
            }
            tableBuilder.append("</tr>");
        }
    ...
    public static  <M>  String tableHTML(List<M> data, String tableClass, String tableId,
        ...
        for (String string:
                attributs) {
            tableBuilder.append("<th>").append(string).append("</th>");
        }
        tableBuilder.append("</tr></thead><tbody>");
        ...
        for (ArrayList<Object> tr : values){
            ...
            for (Object object:
                    tr) {
                tableBuilder.append("<td>");
                tableBuilder.append(objectToString(object));
                tableBuilder.append("</td>");
            }
            tableBuilder.append("</tr>");
        }
    ...
    public static String objectToString(Object object) {
        if (object instanceof Sector)
            return ((Sector) object).wording;

        return object.toString();
    }

    private static String replaceName(Map<String, String> naming, String oldName) {
        return naming.getOrDefault(oldName, oldName);
    }

    public static void _table(Map<?, ?> args, Closure body, PrintWriter out, GroovyTemplate.ExecutableTemplate template, int fromLine) {
    ...
    public static void _column(Map<?, ?> args, Closure body, PrintWriter out, GroovyTemplate.ExecutableTemplate template, int fromLine) {
